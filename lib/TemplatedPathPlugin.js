...
/** @typedef {import("./Compilation").ChunkPathData} ChunkPathData */
...
const replacePathVariables = (path, data) => {
	...
	if (data.chunk) {
		...
		const chunkPathData = /** @type {ChunkPathData} */ (chunk);
		const chunkhashReplacer =
			chunk instanceof Chunk
				? hashLength(replacer(chunk.renderedHash))
				: hashLength(
						replacer(chunkPathData.hash),
						chunkPathData.hashWithLength
				  );
		const contenthashReplacer = data.contentHash
			? hashLength(replacer(data.contentHash), data.contentHashWithLength)
			: chunk instanceof Chunk
			? hashLength(
					replacer(
						contentHashType &&
			...
			: hashLength(
					replacer(
						contentHashType &&
							chunkPathData.contentHash &&
							chunkPathData.contentHash[contentHashType]
					),
					chunkPathData.contentHashWithLength
						? chunkPathData.contentHashWithLength[contentHashType]
						: undefined
			  );
	...
	if (data.module) {
		...
		const modulePathData = /** @type {ModulePathData} */ (module);
		...
		const hashReplacer =
			module instanceof Module
				? hashLength(replacer(chunkGraph.getRenderedModuleHash(module)))
				: hashLength(
						replacer(modulePathData.hash),
						modulePathData.hashWithLength
				  );
